...
cat("EntrezGeneSymbol 的类别数量为:", num_categories, "\n")
...
df_variance <- df_combined %>%
  ...
  pivot_longer(cols = everything(), names_to = "Component", values_to = "Proportion") %>%
  mutate(
    Proportion = Proportion * 100,  # 转换为百分制
    Component = case_when(
      Component == "prop_age" ~ "Age",
      Component == "prop_sex" ~ "Sex",
      Component == "prop_idno" ~ "Individual",
...
# 定义每个Component的自定义y轴范围（转换为百分制后可能需要调整）
y_limits <- list(
  Age = c(0, 5000),       # Age的y轴范围
  Sex = c(0, 10000),        # Sex的y轴范围
  Individual = c(0, 800), # Individual的y轴范围
  Residual = c(0, 800)    # Residual的y轴范围
)
...
p_age <- ggplot(df_variance %>% filter(Component == "Age"), aes(x = Proportion, fill = Component)) +
  ...
  theme(legend.position = "none") +
  ylim(y_limits$Age) +
  scale_x_continuous(labels = scales::percent_format(scale = 1))  # 添加百分号
...
p3_by_gene <- ggplot(df_age_max_by_gene_long, aes(x = Proportion, y = gene_symbol, fill = Component)) +
  geom_col(position = "stack", width = 0.8) +  # 水平堆叠核心
  scale_fill_manual(values = colors_mixed) +  # 使用原有的颜色方案
  scale_x_continuous(
    labels = scales::percent_format(scale = 1)  # 水平轴显示百分比
  ) +
  labs(
    x = "Variance Explained (%)",  # 修改横轴标签
    y = NULL,  # 不显示y轴标签
    fill = "Component"  # 图例标题
  ) +
  theme_bw() +
