...
library(dplyr)
library(tidyr)
library(ggh4x)  # 用于环形图的标签对齐
library(RColorBrewer)  # 用于优化配色
...
# =============================================================================
# 步骤1: 读取both_significant_consistent_direction表格，获取top100的seqid列表
# =============================================================================
cat("步骤1: 读取both_significant_consistent_direction表格...\n")
...
cat("方向一致表格维度:", dim(consistent_df), "\n")
cat("方向一致表格列名:", colnames(consistent_df), "\n")

# 创建label_symbol用于识别重复的gene
...
# 对于重复的gene名称，只保留rank_sum最小的（排名最高的）那一行
cat("\n处理重复的gene名称...\n")
cat("处理前数量:", nrow(consistent_df), "\n")
consistent_df_unique <- consistent_df %>%
  group_by(label_symbol) %>%
  slice_min(rank_sum, n = 1) %>%
  ungroup()

cat("处理后数量:", nrow(consistent_df_unique), "\n")

# 获取top 100 gene（基于rank_sum）
...
cat("\nTop 100 genes的排名范围:", range(top100_genes$rank_sum, na.rm = TRUE), "\n")
cat("Top 100 seqid数量:", length(top100_seqids), "\n")

# =============================================================================
# 步骤2: 读取混合数据（年龄、性别、个体、残差）
# =============================================================================
cat("\n步骤2: 读取混合数据...\n")
...
cat("混合数据表格维度:", dim(mixed_df), "\n")
cat("混合数据表格列名:", colnames(mixed_df), "\n")
